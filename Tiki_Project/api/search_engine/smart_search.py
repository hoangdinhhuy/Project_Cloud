...
class SmartSearch:
    ...
    def search(
        ...
        for i in range(n):
            ...
            if query_has_accent:
                ...
                if query_original and query_original in name_orig:
                ...
                if query_norm and query_norm in name_n:
                    exact_name_score = W_EXACT_PHRASE
                    match_reasons.append("exact_phrase_in_name")
                elif (
                    meaningful_tokens
            ...
            has_signal = (
                ...
                or (result_type != "full_scan" and in_matched_cat)
            )
            if not has_signal:
                continue
    ...
    def _apply_context_filter(
        ...
        for i, score, dinfo in scored:
            name_n = self._name_norms[i]
            name_toks = _token_set(name_n)
            has_main = bool(main_tokens & name_toks) or main in name_n
            has_ctx  = bool(ctx_tokens & name_toks) or ctx in name_n

            if has_main and has_ctx:
