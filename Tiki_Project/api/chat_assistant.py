...
def analyze_market(keyword: str) -> str:
    """
    Phân tích tổng thể một ngách thị trường dựa trên từ khóa tìm kiếm (số đối thủ, dung lượng, khoảng giá, và tính khả thi).
...
"""
    ...
    def analyze_market(self, keyword: str) -> str:
        try:
            logger.info(f"[Service: analyze_market] Keyword: {keyword}")
            products = self.search_engine.search_products(keyword, limit=50)
            if not products:
            ...
            budget_count = len([p for p in products if p["price"] < avg_price * 0.7])
            mid_count = len([p for p in products if avg_price * 0.7 <= p["price"] <= avg_price * 1.3])
            premium_count = len([p for p in products if p["price"] > avg_price * 1.3])
            ...
            market = {
                ...
                "price_segments": {
                    "budget_pct": round(budget_count / len(products) * 100, 1),
                    "mid_pct": round(mid_count / len(products) * 100, 1),
                    "premium_pct": round(premium_count / len(products) * 100, 1)
                },
    ...
    def _execute_tool(self, func_name, args):
        ...
        try:
            if func_name == "recommend_business":
                ...
                return self.analyze_market(keyword=args.get("keyword", ""))
